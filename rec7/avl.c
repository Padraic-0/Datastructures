...
void height_helper(struct avl_node *node){
    node->height = 0;
    node = node->parent;
...
struct avl_node *rightRotate(struct avl_node *N){
    ...
    N->left->parent = N->parent;//update parent
   struct avl_node* new = N->left;
   struct avl_node* alpha = new->right;
   N->left = alpha;
   alpha->parent = N;
   N->parent = new;
   new->right = N;
