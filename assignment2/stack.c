...
void stack_free(struct stack* stack) {
	 ...
	 */
	if (stack != NULL){
	list_free(stack->list);//free the list data
	free(stack);//free pointer to stack
	}
	return;
}
...
int stack_isempty(struct stack* stack) {
	if (stack != NULL){
		return empty_list(stack->list);//use list function to see if list has nodes
	}
	return 1;
}
...
void stack_push(struct stack* stack, void* val) {
	if(stack != NULL){
		list_insert(stack->list,val);//insert val to front of list
	}
	return;
}
