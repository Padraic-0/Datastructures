...
int main(int argc, char const *argv[]) {
  ...
  while (get_user_str(in, MAX_STR_LEN)) {
     ...
     */
    struct stack* s;//creating stack and queue to hold the inputed string
    struct queue* q;
    char* stack_char;
    char* queue_char;
    s = stack_create();
    q = queue_create();
    int length = strlen(in);//using length of string to run for loop, each iteration puts the current char into the stack and queue
    for (int i = 0; i < length; i++){
      if(isalpha(in[i])){
        in[i] = tolower(in[i]);
        stack_push(s,&in[i]);
        stack_char = stack_top(s);
        //printf("%c\n",in[i]);
        queue_enqueue(q,&in[i]);
        }
      }
    while (stack_isempty(s) != 1){//reading from the stack and queue using pop and dequeue, not matching chars means the string isnt a palindrome
      stack_char = stack_pop(s);
      queue_char = queue_dequeue(q);
      if(*stack_char != *queue_char){
        printf("No good\n");
        break;//break loop once mismatch occurs
      }
    }
    stack_free(s);//free stack and queue memorey
    queue_free(q);
    break;//exited to terminate program
  }
