...
struct list* list_create() {
  struct list* list = malloc(sizeof(struct list));//allocate memeory
...
void* first_val(struct list* list){//returns heads value, the first val
  return list->head->val;
}

int empty_list(struct list* list){//checks if head  is NULL, thus empty
  if (list->head == NULL){
    return 1;
  }
  return 0;
...
void* pop_list(struct list* list){//returns heads val, moves head to next node, freeing the previous one
  struct node* temp;
  temp = list->head;
  void* val = temp->val;
  free(list->head);
