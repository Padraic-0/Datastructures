...
struct queue* queue_create() {
	struct queue* queue = malloc(sizeof(struct queue));//a queue is made and its array is built using dynarray creator
...
int queue_isempty(struct queue* queue) {
	 ...
	 */
	if(queue != NULL){
		return dynarray_empty(queue->array);//call dynarray empty function to see if array is empty
	}
	return 1;
}
...
void queue_enqueue(struct queue* queue, void* val) {
	 ...
	 */
	if (queue != NULL){
		dynarray_insert(queue->array,val);//passes val to array insert function
	}
	return;
}
...
void* queue_front(struct queue* queue) {
	 ...
	 */
	if(queue != NULL){
		return dynarray_get(queue->array,0);//uses dyanarray get function to pass on the void*
	}
	return NULL;
}
