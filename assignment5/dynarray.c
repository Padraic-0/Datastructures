...
void lower_size(struct dynarray* da){
  da->size--;
}
 ...
 * storage array.
 */
void _dynarray_resize(struct dynarray* da, int new_capacity, int (*convert)(void*)) {
  ...
  for (int i = 0; i < da->capacity; i++){
    if(da->data[i]!=NULL){
      int index = convert(da->data[i])%new_capacity;
      while(new_data[index]!=NULL){
        index = (index +1 )% new_capacity;
      }
      new_data[index] = da->data[i];
    }
  }
...
void free_element(struct dynarray* da, int idx){
  free(da->data[idx]);
  //da->data[idx] = NULL;
}

void dynarray_clear(struct dynarray* daOld, struct dynarray* daNew){
  free(daOld->data);
  daOld->data = daNew->data;
  daOld->capacity = daNew->capacity;
}
